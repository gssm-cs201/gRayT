...
#define setup(i,b0,b1,r0,r1)\
    ...
    std::shared_ptr<PerlinNoise> PerlinNoise::Ptr() { return std::shared_ptr<PerlinNoise>(new PerlinNoise()); }
    ...
    void PerlinNoise::setTime( const float time ) {}
		...
		const float PerlinNoise::eval( const math::Vector &vec) const
		{
...
#define LERP(t, a, b) ((a) + (t)*((b)-(a)))
    ...
    std::shared_ptr<PerlinNoiseGustavson> PerlinNoiseGustavson::Ptr() { return std::shared_ptr<PerlinNoiseGustavson>(new PerlinNoiseGustavson()); }
    ...
    void PerlinNoiseGustavson::setTime( const float t ) { mImpl->time = t; }
		...
		const float PerlinNoiseGustavson::eval( const math::Vector &P ) const
		{
