...
class CmdLineFind
{
  public:
   ...
   const math::Vector find( const std::string& tag, const math::Vector deflt, const std::string help = "no help" )
   {
      math::Vector result = deflt;
      std::vector<size_t> locate = find( tag );
      if( !locate.empty() && locate[0] < items.size()-1 )
      {
         result = math::Vector(   atof( items[locate[0]+1].c_str() ),   atof( items[locate[0]+2].c_str() ),   atof( items[locate[0]+3].c_str() )  );
      }
      registerTag( tag, deflt, result, help );
      return result;
   }
   ...
   const std::vector<math::Vector> findMultiple( const std::string& tag, const math::Vector deflt, const std::string help = "no help" )
   {
	 ...
	 {
            result.push_back( math::Vector(   atof( items[locate[i]+1].c_str() ),   atof( items[locate[i]+2].c_str() ),   atof( items[locate[i]+3].c_str() )  ) );
	 }
	 else
	 {
  ...
  private:
   ...
   void registerTag( const std::string& tag, const math::Vector& def, const math::Vector& val, const std::string& help )
   {
      cmdLineList.push_back(tag);
   ...
   void registerTag( const std::string& tag, const std::vector<math::Vector>& val, const std::string& help )
   {
      cmdLineList.push_back(tag);
      cmdLineDefaultsList.push_back( "no default" );
